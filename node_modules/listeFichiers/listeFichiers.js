...
module.exports = {
    listeFichiersHTML: function(utilisateur, index){
        ...
        if(utilisateur == 'superadmin'){
            try {
                liste = fs.readdirSync('files/');
                console.log(liste);

                for (i = 0; i < liste.length; i++) {

                    if(liste[i] != 'utilisateurs.xml') {
                        listeFichier = fs.readdirSync('files/'+ liste[i] +'/');
                        fichiers = fichiers + '<div id="separateur" class="dListeFichierSep">'+ liste[i] +'</div>'
                        for (j = 0; j < listeFichier.length; j++) {
                            fileName = listeFichier[j].substring(0, listeFichier[j].lastIndexOf('.'));
                            fichiers = fichiers + '<div id="dFichiers" name="' + fileName + '"';
                            fichiers = fichiers + 'class=' + strID;
                            fichiers = fichiers + '><label>' + fileName + etatFichier(liste[i], fileName) + '</label>';
                            fichiers = fichiers + telecharge(liste[i], index, fileName, true);
                            ...
                            fichiers = fichiers + '<input type="hidden" name="utilisateur" value="'+liste[i]+'">';
            ...
            if (utilisateur != 'admin') {
                try {
                    ...
                    for (i = 0; i < liste.length; i++) {
                        ...
                        fichiers = fichiers + telecharge(utilisateur, index, fileName, false);
...
function telecharge(user, index, fileName, superadm){
    var s = '';
    if((user == 'admin')||(superadm == true)){
